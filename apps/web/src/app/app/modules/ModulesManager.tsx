...
function asModuleList(value: unknown): InstalledModule[] {
  ...
  return (value as InstalledModule[]).filter(
    (m) => m && typeof m === 'object' && typeof (m as InstalledModule).id === 'string'
  )
}
...
export type ModulesScope = 'sfw' | 'nsfw'

export function ModulesManager({ scope }: { scope: ModulesScope }) {
  const showNsfw = scope === 'nsfw'
  ...
  const [testQuery, setTestQuery] = useState(showNsfw ? 'Overflow' : 'Naruto')
  ...
  useEffect(() => {
    if (showNsfw) {
      setTestQuery((q) =>
        q.trim().toLowerCase() === 'naruto' || !q.trim() ? 'Overflow' : q
      )
    }
  }, [showNsfw])
  ...
  const catalogVisible = useMemo(() => {
    return catalog.filter((e) => {
      ...
      if (showNsfw ? !isNsfwEntry(e) : isNsfwEntry(e)) return false
      return true
    })
  }, [catalog, showNsfw])
  ...
  const filteredInstalled = useMemo(() => {
    const base = installed.filter((m) => {
      ...
      if (showNsfw ? !isNsfwEntry(m) : isNsfwEntry(m)) return false
      return true
    })
  ...
  async function move(id: string, dir: -1 | 1) {
    ...
    setBusyId(id)
    try {
      await native.reorderModules(ids)
      await refreshInstalled()
    } catch (e) {
      setStatus(e instanceof Error ? e.message : String(e))
      try {
  ...
  async function installEntry(entry: ModuleCatalogEntry) {
    ...
    setError('')
    try {
      if (isNsfwEntry(entry) && !showNsfw) {
        setError('NSFW modules belong on Adult → Modules (enable Adult Mode first).')
        return
      }
      if (!isNsfwEntry(entry) && showNsfw) {
        setError('SFW modules belong on Settings → Modules.')
        return
      }
      ...
      // Always re-list after install — don't trust a one-shot payload that can
      // blank the pane if Capacitor returns an unexpected shape.
      await refreshInstalled()
      setTab('installed')
  ...
  async function installCatalogEntries(entries: ModuleCatalogEntry[]) {
    ...
    for (const entry of targets) {
      ...
      try {
        if (isNsfwEntry(entry) && !showNsfw) {
          setError('NSFW modules belong on Adult → Modules.')
          continue
        }
        if (!isNsfwEntry(entry) && showNsfw) {
          continue
    ...
    if (looksLikeCatalogUrl(url)) {
      ...
      try {
        ...
        if (showNsfw) {
          ...
          setTab('browse')
          setStatus(
            'Catalog added. NSFW modules appear under Adult → Modules when Adult Mode is on.'
          )
        }
      } catch (e) {
        setError(e instanceof Error ? e.message : String(e))
  ...
  return (
    <>
      <PageHeader
        title={showNsfw ? 'Adult modules' : 'Modules'}
        description={
          showNsfw
            ? 'NSFW stream sources only. Separate from Settings → Modules.'
            : 'SFW stream sources only. NSFW sources live under Adult settings.'
        }
        action={
      ...
      <div className="mt-3 rounded-lg border border-border/50 px-3 py-2.5">
        <p className="text-xs text-muted-foreground">
          {showNsfw
            ? enabledNsfwCount > 0
              ? `${enabledNsfwCount} NSFW source(s) enabled — used by the Adult tab.`
              : 'Install and enable an NSFW source, then set it as primary in Adult settings.'
            : 'This list never shows NSFW modules. Open Adult settings → Adult modules for those.'}
        </p>
      </div>
      ...
      <Sheet open={filtersOpen} onOpenChange={setFiltersOpen}>
        <SheetContent side="bottom" className="max-h-[80dvh] rounded-t-2xl">
          ...
          <div className="space-y-4 px-4 pb-[calc(1rem+var(--safe-bottom))]">
            ...
            <div className="rounded-lg border border-border/50 px-3 py-2.5 text-sm text-muted-foreground">
              {showNsfw
                ? 'This page lists NSFW modules only.'
                : 'This page lists SFW modules only.'}
            </div>
